...
Reader includes or uses software owned by its respective upstream authors. Nothing in this project implies ownership of Mozilla PDF.js, JSZip, Marked, Tauri, Playwright, axe, or their trademarks.
...
The exact upstream tarball URLs, registry integrity values, SHA-256 hashes, vendored filenames, and license-file hashes are recorded in `THIRD_PARTY_COMPONENTS.json`. `pnpm verify:vendor` checks every vendored runtime byte against that manifest and confirms that the PDF.js library and worker have matching version/build identifiers.
