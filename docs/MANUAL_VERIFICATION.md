...
| Tauri GUI subsystem                   | Rust source/PE verification    | Static/binary passed; interactive pending | Release executable uses the Windows GUI subsystem; signed-in-user launch remains a publication gate                              |
| Single instance / one window          | Native source regression       | Static passed; interactive pending        | One manual `main` window, native popup denial, and first-registered single-instance focus are asserted; two-launch smoke remains |
