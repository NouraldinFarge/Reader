...
| Tauri GUI subsystem                   | Rust source/PE verification              | Pending final alpha.4 record | Unconditional Windows GUI subsystem retained                                                  |
| Single instance / one window          | Native source regression and local smoke | Pending final alpha.4 record | Second launch must focus existing `main` window                                               |
| Local NSIS build                      | Local Windows toolchain                  | Pending                      | Must record installer hash and signature status                                               |
...
| Authenticode                          | Owner-approved certificate               | Not run                      | Candidate is unsigned; approval required                                                      |
